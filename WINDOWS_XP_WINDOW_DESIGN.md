...
- 閉じるボタンにカーソルを置いたときは、赤系を維持したまま通常時より少し明るい `linear-gradient(#ffb0a9, #e64c3d 52%, #ba2118)` にする。共通の青ボタン用ホバー色より閉じるボタン専用セレクターの優先度を高くする。
- 最大化を提供しないアプリでも位置を保つため `□` は表示して無効化できる。無効時は半透明・既定カーソルにする。
- 最大化を提供するアプリでは `□` を有効にし、押すたびに最大化と元のサイズへの復元を切り替える。最大化中は復元を示す `❐` 等へ記号と `aria-label` を切り替える。
- タイトルバーのボタン以外を押してドラッグしたときだけ、ホストにウィンドウ移動を依頼する。
- 最大化を提供するアプリでは、タイトルバーのボタン以外をダブルクリックした場合も最大化・復元を切り替える。

## ウィンドウ外周

- 通常表示ではウィンドウ外周を `7px` 程度の軽い角丸にする。Windows 11ではDWMの角丸設定とUI側の `border-radius` を併用する。
- UI側は外枠に `overflow: hidden` を設定し、タイトルバーとクライアント領域が角からはみ出さないようにする。
- 最大化時は画面端に隙間や丸みを残さないよう、UI側の `border-radius` を `0` に切り替える。
- 最大化領域は現在のモニターの作業領域に合わせ、タスクバーを覆わないようにする。
...
.xp-shell {
  ...
  border: 1px solid #052468; border-radius: 7px; background: #c4d7fb;
}
body.maximized .xp-shell { border-radius: 0; }
.xp-titlebar {
...
.xp-caption-buttons button.close:hover {
  border-color: #fff0ec;
  background: linear-gradient(#ffb0a9, #e64c3d 52%, #ba2118);
}
```
...
- 閉じるボタンだけ赤である。
- 閉じるボタンはホバー時に赤系のまま少し明るくなり、青色へ変化しない。
- 通常表示では外周が軽く角丸になり、最大化時は角丸が解除される。
- 最大化ボタンが有効な場合、最大化と復元の両方が動作し、表示記号と読み上げラベルも状態に追従する。
- タイトルバーのダブルクリックで最大化・復元できる。
